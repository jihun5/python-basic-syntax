...
# 버블정렬
lista = [93,45,21,30,20,94,66,71,45]
for a in range(len(lista)): # a는 전체 반복 횟수
    for b in range(len(lista)-1-a): # -1은 맨 마지막 수는 더이상 바꿀 필요가 없기 때문
                                    # -a는 반복횟수를 줄이기 위해 마지막이 고정되고 그 다음 마지막이 고정이 되게 해서
                                    # 반복횟수를 8번 7번 6번 하게 해주는 것이 -a
        if lista[b] > lista[b+1]:   # 이게 가장 핵심 list[0] = 93, list[0+1] = 45일때 b > b+1보다 클때 
            lista[b], lista[b+1] = lista[b+1], lista[b] # b+1과 b를 순서를 바꿔주기 
                                                        # 첫 한바퀴를 9회 반복을 했을 시 94가장 큰 숫자가 맨 오른쪽
                                                        # 다음 횟수 반복 시 8번만 반복 하고 93, 94가 출력이 되는 순서
print(lista)
...
for a in range(len(arr1)): # 전체 리스트의 길이 0~1, 2개의 요소
    temp = [] 
    for b in range(len(arr1[a])): # 그 리스트 안에 길이 1,2, 2개의 요소
